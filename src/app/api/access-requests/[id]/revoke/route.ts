...
export async function PUT(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (access.vehicle.ownerId !== user.id && access.userId !== user.id) return NextResponse.json({ error: "Acesso negado" }, { status: 403 });

    const updated = await db.vehicleAccess.update({ where: { id }, data: { status: "REVOKED" } });
